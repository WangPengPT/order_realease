import{r as u,c as f,v as p,B as h,y as m,A as c,f as b,p as x}from"./index-74z0hs9b.js";import{b as v}from"./index-5wC19fmD.js";const l=u([]),s=u([]),g=e=>{l.value.length=0;for(let t=0;t<e.length;t++)l.value.push(e[t])};function y(){const e=[];for(const t of s.value)for(const a of t.mainDishes){a.dish.quantity>0&&e.push(a.dish);for(const r of a.subItems)r.quantity>0&&(r.name=a.dish.name,r.image=a.dish.image,e.push(r))}return e}function $(e){if(Array.isArray(e))for(const t of e)for(const a of s.value)for(const r of a.mainDishes){r.dish.id===t.id&&r.subItems.length===0&&(r.dish.quantity=t.quantity);for(const d of r.subItems)d.id===t.id&&(d.quantity=t.quantity)}}function z(){for(const e of s.value)for(const t of e.mainDishes){t.dish.quantity=0;for(const a of t.subItems)a.quantity=0}}const k={dishDatas:s,dishes:l,showDishList:g,getAllIncrementedDish:y,updateDishDataQty:$,cleanDishDataQty:z},i=u([]),w=f(()=>{const e=i.value.length,t=k.dishDatas.value.flatMap(a=>a.mainDishes).flatMap(a=>[a.dish,...a.subItems.map(r=>({...r,parentName:a.dish.name,parentImage:a.dish.image}))]).filter(a=>a.quantity>0).reduce((a,r)=>a+r.quantity,0);return e+t});function D(e){e.id=q(),i.value.push(e)}function I(e){for(let t=0;t<i.value.length;t++)if(i.value[t].id==e.id){i.value.splice(t,1);break}}function q(){const e=Date.now().toString(36),t=Math.random().toString(36).substring(2,5);return`${e}-${t}`}function P(){i.value=[]}const E={addItem:D,removeItem:I,cartDishs:i,cartItemCount:w,cleanAll:P};var S=p`
    .p-textarea {
        font-family: inherit;
